...
if(isset($_POST["division_id"]) && !empty($_POST["division_id"])){
  ...
  $sqlSite = "SELECT s.id, s.name, s.short_name, s.site_map_img FROM `site` s, user_sites u WHERE s.id = u.site_id AND u.user_id = ".$userId." AND s.division_id =".$_POST['division_id']." ORDER BY s.name";
...
try {
	...
	echo "<select class='form-control form-control-smX site_change' id='site' name='site'>";
...
?>
	...
	<div class="input-group-append">
		<button class="btn btn-outline-secondary viewMapImage" type="button"><span class="oi oi-map"></span</button>
	</div>	
	</div>	
...
$(".viewMapImage").click(function(e) {

	var imgNm = $( "#loc_img" ).data('locimg');
	var site = $('#iSite').val();
	
	
	if (site !== ""){
		document.getElementById('input_img_name').value =imgNm;
		$("#ItemImgModal_View").modal('show');
	}
