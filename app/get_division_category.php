...
if(isset($_POST["division_id"]) && !empty($_POST["division_id"])){
	...
	if($categoryid>0){
		...
		if($categorytype=='a'){
			...
			$sqlCat = "SELECT id,category_name, 'a' as type FROM `asset_category` WHERE division_id =".$_POST['division_id']." AND id =".$categoryid.";";
		
		}else{
			
			$sqlCat = "select id,category_name, 'p' as type from product_category WHERE division_id =".$_POST['division_id']." AND id =".$categoryid.";";
