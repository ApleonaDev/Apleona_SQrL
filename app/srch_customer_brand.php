...
if(isset($_POST["customer_id"]) && !empty($_POST["customer_id"])){
	if($catType == 'a'){
		...
		$sql_nm = "SELECT distinct at.name as item_name FROM `asset_type` as at WHERE customer_id =".$_POST['customer_id']." AND category_id =".$_POST['catId']." order by at.name";
	}else if($catType == 'p'){
		...
		$sql_nm = "SELECT distinct pt.name as item_name FROM `product_type` as pt WHERE customer_id =".$_POST['customer_id']." AND category_id =".$_POST['catId']." order by pt.name";	
	}
}
