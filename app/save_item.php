...
$iBNPrice = empty($_POST["input_bn_price"]) ? "0" : trim($_POST["input_bn_price"]);	
$iSPrice = empty($_POST["input_selling_price"]) ? "0" : trim($_POST["input_selling_price"]);	
$iASPrice = empty($_POST["input_alt_selling_price"]) ? "0" : trim($_POST["input_alt_selling_price"]);
...
$iCrateNo = empty($_POST["input_crate_number"]) ? "0" : $_POST["input_crate_number"];
...
if ($iId){
	...
	if($itype=='a'){//Update Asset type details
		...
		$update_type = "UPDATE asset_type SET description=:iDesc,finish=:iFinish,dimensions=:iDimensions,cost=:iCost,currency=:iCurrency,customer_id=:iCustomerId,maunufacturer_id=:iManuId,owner_id=:iCltId,
		`name`=:iName, buy_new_price=:iBNPrice, selling_price=:iSPrice, alt_selling_price=:iASPrice WHERE id = :iId;";
		try {
			$stmt = $conn->prepare($update_type);  
				...
				$stmt->bindParam(':iBNPrice', $iBNPrice, PDO::PARAM_STR);
				$stmt->bindParam(':iSPrice', $iSPrice, PDO::PARAM_STR);
				$stmt->bindParam(':iASPrice', $iASPrice, PDO::PARAM_STR);
		...
		$update_loc = "UPDATE asset_location SET loc_code=:iLocCode,site_id=:iSiteId,site_map_img=:iLocMap,building_id=:iBldId,floor_id=:iFloorId,room_id=:iRoomId,area_id=:iAreaId,rack_id=:iRackId,
		shelf_id=:iShelfId, crate_number=:iCNo WHERE id =:locId AND asset_id =:assetId AND asset_type_id = :iId";
		try {
			$stmtLoc = $conn->prepare($update_loc);     
				...
				$stmtLoc->bindParam(':iCNo', $iCrateNo, PDO::PARAM_STR);
	...
	if($itype=='a'){
		...
		$teststr = "INSERT INTO `asset_type`( `maunufacturer_id`, `name`,  `dimensions`, `finish`, `description`,  `currency`, `cost`, `category_id`, 
		`customer_id`, `owner_id`, `division_id`, buy_new_price, selling_price, alt_selling_price) 
		VALUES ('$_POST[input_item_manu]', trim('$_POST[inm]'), trim('$_POST[input_item_dimensions]'), trim('$_POST[input_item_finish]'), trim('$_POST[input_item_desc]'), $_POST[input_item_currency], 
		$iCost, $_POST[iCatId], $_POST[customer], $_POST[input_item_owner], $_POST[division], $iBNPrice, $iSPrice, $iASPrice);";
		 
		 
		
		
		$insert_type = "INSERT INTO asset_type (division_id, maunufacturer_id,owner_id,name,dimensions,description,finish,currency,cost,customer_id,category_id,buy_new_price, selling_price, alt_selling_price, barcode,last_updated) 
		VALUES (:iDivId,:iManuId,:iCltId,:iName,:iDimensions,:iDesc,:iFinish,:iCurrency,:iCost,:iCustomerId,:iCategoryId, :iBNPrice, :iSPrice, :iASPrice,:iBarcode, now())";
		try {  
			$stmt = $conn->prepare($insert_type);     
				$stmt->bindParam(':iDivId', $_POST['division'], PDO::PARAM_INT);
				$stmt->bindParam(':iName',trim($_POST['inm']), PDO::PARAM_STR);
				$stmt->bindParam(':iDesc',trim($_POST['input_item_desc']), PDO::PARAM_STR);
				$stmt->bindParam(':iFinish', trim($_POST['input_item_finish']), PDO::PARAM_STR);
				$stmt->bindParam(':iDimensions', trim($_POST['input_item_dimensions']), PDO::PARAM_STR);
		...
		if($asset_id>0){
			//echo '-asset_id-'.$asset_id;crate_number
			...
			try {
				$stmt3 = $conn->prepare($insert_asset_loc); 
					...
					//$stmt3->bindParam(':iCNo', $iCrateNo, PDO::PARAM_STR);
...
echo '-product-';
		$insert_type = "INSERT INTO product_type (`vendor_id`, `maunufacturer_id`, `name`,  `description`, `barcode`, `currency`, `cost`,  `category_id`, `unit_amount`, `customer_id`, `unit_id`,  `last_updated`) 
		VALUES (:iVendorId,:iManuId,:iName,:iDesc,:iBarcode,:iCurrency,:iCost,:iCategoryId,:iUAmt,:iCustomerId,:iUId,now())";
		/*try {  
			$stmt = $conn->prepare($insert_type);     
				$stmt->bindParam(':iName',trim($_POST['inm']), PDO::PARAM_STR);
				$stmt->bindParam(':iDesc',trim($_POST['input_item_desc']), PDO::PARAM_STR);
